...
function pump(){if(mode==='none'||loading||!Number.isFinite(video.duration))return;const base=Math.floor(video.currentTime/2)*2;
...
function tick(){if(mode!=='none'&&!loading&&Number.isFinite(video.duration)){
    ...
    }else if(wanted&&!video.seeking){const next=records.get(base+2);if(time>=base+1.94&&(!next||next.status!=='classified'||next.decision==='skip')){hold('Segment boundary…');video.currentTime=Math.min(base+2,video.duration);}
